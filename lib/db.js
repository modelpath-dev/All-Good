...
export async function setupDatabase() {
  ...
  await sql`
    ALTER TABLE activity_logs ADD COLUMN IF NOT EXISTS location VARCHAR(200) DEFAULT ''
  `
...
export function logActivity(ip, action, details = '') {
  const sql = getSQL()
  fetch(`http://ip-api.com/json/${ip}?fields=city,regionName,country`)
    .then(r => r.json())
    .then(geo => {
      const location = geo.city ? `${geo.city}, ${geo.regionName}, ${geo.country}` : ''
      sql`INSERT INTO activity_logs (ip, action, details, location) VALUES (${ip}, ${action}, ${details}, ${location})`.catch(() => {})
    })
    .catch(() => {
      const noLoc = ''
      sql`INSERT INTO activity_logs (ip, action, details, location) VALUES (${ip}, ${action}, ${details}, ${noLoc})`.catch(() => {})
    })
}

export async function getActivityLogs(limit = 200) {
  const sql = getSQL()
  return await sql`SELECT * FROM activity_logs ORDER BY created_at DESC LIMIT ${limit}`
}
